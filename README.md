...
* 增加如下新的核心函数:
  ...
  * `keyword`: 接受一个字符串，返回有相同 name 的 keyword（通常是有前缀的特殊unicode符号）这个函数也被用来判断参数是否已经是一个 keyword，并返回它。
...
如果你推迟了 keywords，vectors 和 hash-map 的实现，那么如果你想实现自足执行的话，现在就需要先回去把这些任务完成。
